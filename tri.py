...
# API 4 (THIRDWAVE)
API_4_URL = "https://app.thirdwave.im/api/v1/numbers"
API_4_TOKEN = "tw_live_b513e8a03b635b5f79f9655821e092ccd9138c316cb61e6a7wbdae20851cf8e0"
...
# HTTP Session Global untuk menjaga koneksi TCP tetap terbuka (hemat latency & resource)
...
def fetch_api_4():
    ...
    try:
        ...
        data = res.json()
        messages = data.get("rows") or data.get("data") or data.get("messages") or [] if isinstance(data, dict) else data
        if messages:
...
# ==============================================================================
# TELEGRAM BOT LISTENER (HANDLERS)
# ==============================================================================
    ...
    while True:
        try:
            ...
            for update in updates:
                ...
                # -------------------------------------------------------------
                # 1. CALLBACK QUERY HANDLER (Tombol Inline Klik)
                # -------------------------------------------------------------
                ...
                # -------------------------------------------------------------
                # 2. MESSAGE HANDLER (Pesan Teks & Dokumen)
                # -------------------------------------------------------------
                if "message" in update:
                    ...
                    # --- COMMAND KHUSUS ADMIN PRIVAT ---
                    if chat_type == "private":
                        if user_id not in ADMIN_IDS:
                            continue

                        # A. Tambah Nomor via Teks: /add <NEGARA> <NOMOR1> <NOMOR2> ...
                        ...
                        # B. Hapus Nomor: /del <NOMOR/NEGARA> atau /dell <NEGARA>
                        ...
                        # C. Setting Link Group/Channel OTP: /setchannel <LINK>
                        ...
                        # D. Handling Upload File Admin (.txt / .xlsx)
                        if "document" in message:
                            doc = message["document"]
                            file_id = doc["file_id"]
                            file_name = doc.get("file_name", "file").lower()

                            # 1. Excel Converter (.xlsx)
                            if file_name.endswith(".xlsx"):
                                handle_file_conversion(chat_id, file_id, file_name)
                                continue

                            # 2. Import Nomor TXT (.txt) via Caption /add <NEGARA>
...
if __name__ == "__main__":
    ...
    # Jalankan background threads untuk pemantauan API & Laporan
    ...
    # Jalankan Telegram Listener (Main Thread)
    telegram_listener()
